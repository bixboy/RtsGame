...
UWorld* UGameUIPolicy::GetWorld() const
{
    ...
    if (!UIManager)
    {
        UE_LOG(LogTemp, Error, TEXT("UGameUIPolicy::GetWorld() - GetOwningUIManager() est nullptr!"));
        return nullptr;
    }
    ...
    if (!GameInstance)
    {
        UE_LOG(LogTemp, Error, TEXT("UGameUIPolicy::GetWorld() - GetGameInstance() est nullptr!"));
        return nullptr;
    }
